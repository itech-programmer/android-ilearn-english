...
import uz.qubemelon.ilearn.adapters.courses.questions.VocabularyQuestionsAnswersAdapter;
...
public class FragmentVocabularyQuestions extends Fragment {
    ...
    private VocabularyQuestionsAnswersAdapter questions_answers_adapter;
    private RecyclerView answer_recycler_view;
    public int TOTAL_POINT = 0;
    private View view;

    public FragmentVocabularyQuestions() {
        // Required empty public constructor
    }

    public static FragmentVocabularyQuestions new_instance(QuestionList item) {
        Bundle bundleGot = new Bundle();
        bundleGot.putSerializable(Utility.QUESTION, item);
        FragmentVocabularyQuestions fragment = new FragmentVocabularyQuestions();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (getActivity() != null && isAdded()) {
            ...
            text_question_position.setText(String.format("%d", ((VocabularyQuestionsActivity) getActivity()).x));
        ...
        if (getArguments() != null) {
            ...
            if (question_item != null) {
                ...
                text_question_count.setText(String.format("/%d", ((VocabularyQuestionsActivity)  getActivity()).question_list.getQuestionList().size()));
                /*make the option recycler view*/
                text_question_name.setText(question_item.getTitle());
                questions_answers_adapter = new VocabularyQuestionsAnswersAdapter(this, Utility.remove_empty_fields(question_item.getAnswers()), getActivity(), question_item.getId(), question_item.getPoint(), img_cat_status, text_point);
        ...
        btn_next.setOnClickListener(view -> {
            ...
            try {
                if (VocabularyQuestionsActivity.isPlayed.get(question_item.getId())) {
                    go_to_next_question();
                } else {
    ...
    private void go_to_next_question() {
        ...
        if (getActivity() != null && isAdded()) {
            if (((VocabularyQuestionsActivity) getActivity()).question_list != null) {
                if (((VocabularyQuestionsActivity) getActivity()).x
                        < ((VocabularyQuestionsActivity) getActivity()).question_list.getQuestionList().size()) {
                    FragmentVocabularyQuestions fragment_questions = new FragmentVocabularyQuestions();
                    Bundle bundle = new Bundle();
                    if (getActivity() != null)
                        bundle.putSerializable(Utility.QUESTION, ((VocabularyQuestionsActivity) getActivity()).question_list.getQuestionList().get(((VocabularyQuestionsActivity) getActivity()).x++));
                    bundle.putInt("TOTAL_POINT", TOTAL_POINT);
                    fragment_questions.setArguments(bundle);

                    ((VocabularyQuestionsActivity) getActivity()).fragment_transition(fragment_questions);
    ...
    private void time_count(long milliseconds) {
        if (getActivity() != null)
            VocabularyQuestionsActivity.count_down_timer = new CountDownTimer(milliseconds, 1000) {
                @Override
        ...
        VocabularyQuestionsActivity.count_down_timer.start();
    }
    ...
    public void onDestroyView() {
        if (getActivity() != null)
            if (VocabularyQuestionsActivity.count_down_timer != null) {
                VocabularyQuestionsActivity.count_down_timer.cancel();
            }
        super.onDestroyView();
    }
