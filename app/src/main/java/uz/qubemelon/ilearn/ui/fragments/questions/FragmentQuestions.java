...
import uz.qubemelon.ilearn.adapters.courses.questions.QuestionsAnswersAdapter;
...
public class FragmentQuestions extends Fragment {
    ...
    private CourseResponse question_list;
    private QuestionsAnswersAdapter questions_answers_adapter;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (getActivity() != null && isAdded()) {
            ...
            text_question_position.setText(String.format("%d", ((QuestionsActivity) getActivity()).x));
        ...
        if (getArguments() != null) {
            ...
            if (question_item != null) {
                ...
                text_question_count.setText(String.format("/%d", ((QuestionsActivity)  getActivity()).question_list.getQuestionList().size()));
                /*make the option recycler view*/
                text_question_name.setText(question_item.getTitle());
                questions_answers_adapter = new QuestionsAnswersAdapter(this, Utility.remove_empty_fields(question_item.getAnswers()), getActivity(), question_item.getId(), question_item.getPoint(), img_cat_status, text_point);
        ...
        btn_next.setOnClickListener(view -> {
            ...
            try {
                if (QuestionsActivity.isPlayed.get(question_item.getId())) {
                    go_to_next_question();
                } else {
    ...
    private void go_to_next_question() {
        ...
        if (getActivity() != null && isAdded()) {
            if (((QuestionsActivity) getActivity()).question_list != null) {
                if (((QuestionsActivity) getActivity()).x
                        < ((QuestionsActivity) getActivity()).question_list.getQuestionList().size()) {
                    FragmentQuestions fragment_questions = new FragmentQuestions();
                    Bundle bundle = new Bundle();
                    if (getActivity() != null)
                        bundle.putSerializable(Utility.QUESTION, ((QuestionsActivity) getActivity()).question_list.getQuestionList().get(((QuestionsActivity) getActivity()).x++));
                    bundle.putInt("TOTAL_POINT", TOTAL_POINT);
                    fragment_questions.setArguments(bundle);

                    ((QuestionsActivity) getActivity()).fragment_transition(fragment_questions);
